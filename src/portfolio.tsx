  ...
  return (
    <div className="min-h-screen transition-colors duration-500" style={{ 
      ...
      <style>{`
        ...
        :root { --bg-primary: #0a0e17; --bg-secondary: #151921; --bg-tertiary: #1e2433; --accent-primary: #00d4ff; --accent-secondary: #7c3aed; --accent-glow: #0ea5e9; --text-primary: #e2e8f0; --text-secondary: #94a3b8; --border-color: rgba(0, 212, 255, 0.2); }
        @media (prefers-color-scheme: light) { :root { --bg-primary: #f8fafc; --bg-secondary: #ffffff; --bg-tertiary: #e2e8f0; --accent-primary: #0891b2; --accent-secondary: #7c3aed; --accent-glow: #0ea5e9; --text-primary: #0f172a; --text-secondary: #475569; --border-color: rgba(8, 145, 178, 0.3); } }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .neon-border { border: 2px solid var(--border-color); box-shadow: 0 0 20px var(--border-color), inset 0 0 20px rgba(0, 212, 255, 0.1); }
        .glass-effect { background: rgba(15, 23, 42, 0.6); backdrop-filter: blur(20px); border: 1px solid var(--border-color); }
        @media (prefers-color-scheme: light) { .glass-effect { background: rgba(255, 255, 255, 0.7); } }
        .floating { animation: floating 3s ease-in-out infinite; }
        @keyframes floating { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-20px); } }
        .pulse-glow { animation: pulseGlow 2s ease-in-out infinite; }
        @keyframes pulseGlow { 0%, 100% { box-shadow: 0 0 20px var(--accent-glow), 0 0 40px var(--accent-glow); } 50% { box-shadow: 0 0 40px var(--accent-glow), 0 0 80px var(--accent-glow); } }
        .scan-line { position: relative; overflow: hidden; }
        .scan-line::before { content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%; background: linear-gradient(90deg, transparent, var(--accent-primary), transparent); animation: scan 3s linear infinite; opacity: 0.3; }
        @keyframes scan { 0% { left: -100%; } 100% { left: 100%; } }
        .grid-bg { background-image: linear-gradient(var(--border-color) 1px, transparent 1px), linear-gradient(90deg, var(--border-color) 1px, transparent 1px); background-size: 50px 50px; }
        .tech-card { transition: all 0.3s ease; border: 1px solid var(--border-color); }
        .tech-card:hover { transform: translateY(-5px) scale(1.02); box-shadow: 0 20px 40px rgba(0, 212, 255, 0.3); border-color: var(--accent-primary); }
        .skill-bar { position: relative; overflow: hidden; }
        .skill-bar::after { content: ''; position: absolute; top: 0; left: 0; height: 100%; width: 100%; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.3), transparent); animation: shimmer 2s infinite; }
        @keyframes shimmer { 0% { transform: translateX(-100%); } 100% { transform: translateX(100%); } }
        .project-card { transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); }
        .project-card:hover { transform: translateY(-10px); box-shadow: 0 30px 60px rgba(0, 212, 255, 0.4); }
        .tab-button { position: relative; transition: all 0.3s ease; }
        .tab-button::before { content: ''; position: absolute; bottom: 0; left: 50%; width: 0; height: 2px; background: var(--accent-primary); transition: all 0.3s ease; transform: translateX(-50%); }
        .tab-button.active::before { width: 100%; }
        .circuit-line { position: absolute; width: 2px; height: 100px; background: linear-gradient(180deg, transparent, var(--accent-primary), transparent); animation: circuit 3s linear infinite; }
        @keyframes circuit { 0% { transform: translateY(-100px); opacity: 0; } 50% { opacity: 1; } 100% { transform: translateY(100vh); opacity: 0; } }
        .pfp-playing { animation: pulseGlow 2s ease-in-out infinite, pfpSpin 8s linear infinite !important; }
        @keyframes pfpSpin { 0% { border-color: #00d4ff; box-shadow: 0 0 20px #00d4ff, 0 0 40px #00d4ff; } 33% { border-color: #7c3aed; box-shadow: 0 0 20px #7c3aed, 0 0 40px #7c3aed; } 66% { border-color: #0ea5e9; box-shadow: 0 0 20px #0ea5e9, 0 0 40px #0ea5e9; } 100% { border-color: #00d4ff; box-shadow: 0 0 20px #00d4ff, 0 0 40px #00d4ff; } }
        .pfp-container { cursor: pointer; transition: transform 0.2s ease; }
        .pfp-container:active { transform: scale(0.95); }
        @media (pointer: coarse) { .cursor-pointer, button, a, [role="button"], .tab-button, .social-link, .project-link, .view-all-button, .contact-link { cursor: default !important; } }
        @media (pointer: fine) { .cursor-pointer, button, a, [role="button"], .tab-button, .social-link, .project-link, .view-all-button, .contact-link { cursor: pointer; } }
        @media (max-width: 614px) {
          .header-nav { flex-direction: column !important; gap: 16px !important; }
          .header-logo { font-size: 22px !important; }
          .header-logo svg { width: 24px !important; height: 24px !important; }
          .nav-tabs { gap: 16px !important; flex-wrap: wrap; justify-content: center; }
          .tab-button { font-size: 13px !important; letter-spacing: 1px !important; }
          .profile-picture-container { width: 140px !important; height: 140px !important; }
          .profile-badge { padding: 8px !important; top: -5px !important; right: -5px !important; }
          .profile-badge svg { width: 20px !important; height: 20px !important; }
          .main-title { font-size: 32px !important; }
          .main-subtitle { font-size: 16px !important; }
          .social-links { flex-direction: column !important; gap: 12px !important; width: 100%; }
          .social-link { width: 100%; justify-content: center; padding: 10px 16px !important; }
          .section-title { font-size: 28px !important; }
          .section-title svg { width: 22px !important; height: 22px !important; }
          .about-section { padding: 24px !important; }
          .about-text { font-size: 15px !important; }
          .current-focus-title { font-size: 22px !important; }
          .current-focus-list { font-size: 15px !important; }
          .projects-title { font-size: 32px !important; }
          .projects-title svg { width: 28px !important; height: 28px !important; }
          .search-input { font-size: 15px !important; padding: 12px 16px !important; }
          .projects-grid { grid-template-columns: 1fr !important; }
          .project-card { padding: 20px !important; }
          .project-title { font-size: 18px !important; }
          .project-description { font-size: 14px !important; min-height: auto !important; }
          .project-language { font-size: 12px !important; padding: 4px 12px !important; }
          .project-link { font-size: 12px !important; padding: 6px 12px !important; }
          .skills-section-title { font-size: 32px !important; }
          .skill-card { padding: 20px !important; }
          .skill-icon { font-size: 24px !important; }
          .skill-name { font-size: 18px !important; }
          .skill-percentage { font-size: 16px !important; }
          ...
          .contact-description { font-size: 16px !important; }
          .contact-link { padding: 14px 12px !important; font-size: 13px !important; flex-direction: column !important; gap: 8px !important; }
          .contact-link svg { width: 18px !important; height: 18px !important; }
          .contact-link span { word-break: break-word; text-align: center; }
          .footer-text { font-size: 14px !important; }
          .footer-subtext { font-size: 12px !important; }
          .main-content { padding: 40px 16px !important; }
          .repos-summary { padding: 24px 16px !important; }
          .repos-text { font-size: 15px !important; }
          .view-all-button { padding: 12px 20px !important; font-size: 14px !important; }
        }
      `}</style>
...
//         /* Ensure these elements only show pointer on mouse devices */
//         @media (pointer: fine) {
//           .cursor-pointer,
//           button,
//           a,
